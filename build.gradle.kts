...
// The plugin's own version, baked in at build time instead of looked up from the platform.
//
// Every descriptor-lookup API we tried is @ApiStatus.Internal on 2026.2 and gets the upload
// rejected by Marketplace: PluginManagerCore.getPlugin(PluginId) (which sank 2026.2.1) and its
// suggested replacement PluginManager.getPluginByClass. Chasing a still-public variant is
// whack-a-mole against a policy that keeps tightening, and the value is ours -- we already write it
// into plugin.xml from this same property, so there is nothing to ask the platform for. This also
// guarantees the string in the tool window matches the <version> in the descriptor.
val generatePluginVersion by tasks.registering {
    val version = providers.gradleProperty("pluginVersion")
    val outDir = layout.buildDirectory.dir("generated/source/pluginVersion")
    inputs.property("pluginVersion", version)
    outputs.dir(outDir)
    doLast {
        val file = outDir.get().asFile.resolve("com/github/brannow/phpstormmcp/PluginVersion.kt")
        file.parentFile.mkdirs()
        file.writeText(
            """
            package com.github.brannow.phpstormmcp

            // GENERATED by the Gradle task `generatePluginVersion` from gradle.properties.
            // Do not edit -- overwritten on every build.
            internal const val PLUGIN_VERSION: String = "${version.get()}"
            """.trimIndent() + "\n"
        )
    }
}
...
intellijPlatform {
    ...
    // Verify across the WHOLE range we claim, not just the floor. This used to check 261.* only,
    // which is how 2026.2.1 shipped with PluginManagerCore.getPlugin and got rejected by
    // Marketplace: that method is not internal on 261, but is on a later build. Marketplace runs
    // the verifier against every IDE in the declared range, so a floor-only check is strictly
    // weaker than the gate the plugin actually has to pass.
    //
    // With an open ceiling, "the whole range" has no upper bound, so this has to be extended by
    // hand as majors ship — keep the untilBuild here at the newest released major, and bump it
    // together with the matching runIde task.
    //
    // The two checks are complementary; neither subsumes the other:
    //   - verifyPlugin catches API-POLICY problems (internal/removed API) that run perfectly well.
    //     2026.2.1 launched cleanly on 2026.2 and was rejected by Marketplace at the same time.
    //   - launching catches RUNTIME breakage the verifier cannot see, because it never loads a
    //     class or exercises the bundled libraries. That is how 0.7.0 passed and failed to bind.
    // Green here does not mean the plugin runs; a clean launch does not mean it will be accepted.
    pluginVerification {
        ides {
